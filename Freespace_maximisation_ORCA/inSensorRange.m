...
%
    ...
    %{
    if norm(obstacle.velocity) <= 0.05 && distance == 1 && obstacle.gflag == 1
      out = 1;
    end
     %}
    ...
    if dot(goal_dir,relative_dist)/(norm_rel_dist*goal_dist) < 0.86 && goal_dist < 0.5*agent.sensorRange
    out = 0;
    end
    
    if obstacle.gflag == 1 && dot(goal_dir,relative_dist)/(norm_rel_dist*goal_dist) < 0.86
        out = 0;
        
    end
   
    if dot(agent.velocity,relative_dist) < 0 && norm(obstacle.velocity) < 0.01
        out = 0;
    end
    %
    if sqrt(sum((agent.position - obstacle.position).^2)) <= 5*agent.radius 
        out =1;
    end
    %}
end