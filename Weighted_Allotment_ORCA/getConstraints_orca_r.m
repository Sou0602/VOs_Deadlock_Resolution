...
%getConstraints_orca_r - Derives cconstraints for the agents that are
%alloted to be on the right side of the agent
%
% Syntax: [c ceq] = getConstraints_orca_r(agents,i,control,dt)
%
    c = [];
    ceq = [];
    cflag = 0;
    ...
    elseif tp1 > 3*pi/2 && tp1 < 2*pi
        ...
        %collision cone.
            ...
            %Opposite side - enforce the alloted side constraint
            for p = 1:length(agents(i).lws)
            if agents(i).lws(p) ~= 0
            if oname == agents(i).lws(p)
            u = d2*nM(2,:);
            nor = nM(2,:); 
            end
            end
            ...
            %}
            if agents(i).fflag == 1
                %% If forced flag = 1 , only choose the alloted side and not the orca side.
            %
            u = d2*nM(2,:);
            nor = nM(2,:);
            %
