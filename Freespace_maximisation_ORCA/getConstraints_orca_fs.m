function [c, ceq] = getConstraints_orca_fs(agents,i, control, dt)
    ...
    elseif tp1 > 3*pi/2 && tp1 < 2*pi
        ...
        %collision cone.
            if(pAblen>r)
              ...
              if agents(i).sides(k) == 0
            u = d2*nM(2,:);
            nor = nM(2,:); 
              elseif agents(i).sides(k) == 1
            u = d1*nM(1,:);
            nor = nM(1,:); 
              end
              ...
              c(end+1) = -1*(control(1)^2 + control(2)^2 - agents(i).vmax^2*0.04);
